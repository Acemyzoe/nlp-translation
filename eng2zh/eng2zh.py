...
data_path = './data/recmn.txt'
model_save = 'model/model_save'
encoder_save = './model/encoder_save'
decoder_save = './model/decoder_save'
#读取数据
...
def train_model(tags):
    ...
    if tags:
        ...
        model_train.save(model_save)
        encoder_infer.save(encoder_save)
        decoder_infer.save(decoder_save)
        ...
        model_train = load_model(model_save)
        encoder_infer = load_model(encoder_save)
        decoder_infer = load_model(decoder_save)
        for i in range(1010,1030):
            test = encoder_input[i:i+1,:,:]#i:i+1保持数组是三维
            out = predict_chinese(test,encoder_infer,decoder_infer,OUTPUT_LENGTH,OUTPUT_FEATURE_LENGTH)
            print(input_texts[i])
            print(out)
